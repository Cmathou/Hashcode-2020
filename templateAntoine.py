...
def main(fileNbr):
	...
	while nbJour < daysNumber and libDone < libNumber and deletedBooks < bookNumber:
		...
		LibBySignUp, ScoreBySignUp, BookToSend = CreaListes(libList, nbJoursSignupList, BooksPerDayList, 
															BookScores, libNumber, nbJour, 
															daysNumber, LibSign)

		# LibScore, BookToSend = CalcLibScore(libList, nbJoursSignupList, BooksPerDayList, BookScores, libNumber, nbJour, daysNumber )
		indexBestSignup = ChooseBestLib(ScoreBySignUp, LibBySignUp)


		Flag, indexes, books = CheckX2(LibBySignUp, ScoreBySignUp, BookToSend, 
									indexBestSignup, libList, BookScores, nbJoursSignupList, 
